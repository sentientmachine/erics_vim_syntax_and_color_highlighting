...
syn keyword plsqlSQLKeyword DATA_BASE DBA DEFAULT DELAY DELETE DISTINCT
...
syn keyword plsqlSQLKeyword PCTFREE PRIMARY PRIOR PRIVATE PRIVILEGES PUBLIC REFERENCES
...
syn keyword plsqlSQLKeyword SYSDATE TABLES TABLESPACE TEMPORARY TO TREAT
...
syn keyword plsqlKeyword ANY ARRAY ASSIGN AS AT AUTHID BEGIN BODY BY
...
syn keyword plsqlKeyword MEMBER NEW NOCOPY NUMBER_BASE OBJECT OCICOLL
...
syn keyword plsqlKeyword TIMEZONE_ABBR TIMEZONE_HOUR TIMEZONE_MINUTE
...
" Eric, rolling my own more effective float matcher
syn match plsqlFloatLiteral "\<\d\+\.\d*\([Ee][-+]\=\d\+\)\="
syn match plsqlFloatLiteral "\<\.\d\+\([Ee][-+]\=\d\+\)\="
syn match plsqlFloatLiteral "\<\d\+[Ee][-+]\=\d\+"
...
" The built-in types.
syn keyword plsqlStorage ANYDATA ANYTYPE BFILE BINARY_INTEGER BLOB BOOLEAN BIGINT SERIAL
...
syn keyword plsqlStorage SECOND SIGNTYPE SMALLINT STRING SYS_REFCURSOR TIME
syn keyword plsqlStorage TEXT TIMESTAMP TIMESTAMP_UNCONSTRAINED
...
if exists("c_no_bracket_error")
  syn region plsqlParen transparent matchgroup=Delimiter start='(' matchgroup=Delimiter end=')' contains=ALLBUT,@plsqlParenGroup
  syn match plsqlParenError ")"
  syn match plsqlErrInParen contained "[{}]"
else
  syn region plsqlParen transparent matchgroup=Delimiter start='(' matchgroup=Delimiter end=')' contains=ALLBUT,@plsqlParenGroup,plsqlErrInBracket
  syn match plsqlParenError "[\])]"
  syn match plsqlErrInParen contained "[{}]"
...
hi def link plsqlParen          BrownParenSymbols
...
hi def link plsqlSymbol         BrownParenSymbols
